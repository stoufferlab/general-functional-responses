...
colnames(AICcs) <- sub('R','LR',colnames(AICcs))
...
# Hack to rename R -> LR (ratio -> linear ratio)
colnames(MADs) <- sub('R','LR',colnames(MADs))
...
filename <- '../../../../results/R/OnePredOnePrey_figs/OnePredOnePrey_AICc_and_MAD_ranks'
...
tab_Cnt <- rbind(c(rep('',3),'$AIC_c$',rep('',4)),
                 tab_Cnt_AICc, 
...
filename <- '../../../../results/R/OnePredOnePrey_figs/OnePredOnePrey_AICc_and_MAD_toprankBySS'
# pdf(paste0(filename,'.pdf'),
#     height=4,width=5)
postscript(paste0(filename,'.eps'),
           height=4,width=5)
