...
RMSE.SN1 <- unlist(lapply(ffr.fits, function(x){ x$RMSE['Stouffer.Novak.I'][[1]][stat]}))

RMSEs <- data.frame(RMSE.H1, RMSE.H2, RMSE.BD, RMSE.CM, RMSE.SN1)
...
# Rank order
#~~~~~~~~~~~
pdf('../../../../results/R/OnePredOnePrey_figs/OnePredOnePrey_RMSE_ranks_dbs.pdf',height=6,width=2.25)
  par(mar=c(3,7,2.5,0.5), mgp=c(1.5,0.2,0), tcl=-0.1, las=1, cex=0.7, yaxs='i')
    plot(1:nrow(rnkRMSEs), 1:nrow(rnkRMSEs),
         type='n', yaxt='n',
         xlim=c(0.5,ncol(rnkRMSEs)+0.5),
    ...
    box(lwd=1)
    par(xpd=TRUE)
    legend(0.425,nrow(rnkRMSEs)+6,legend=colnames(rnkRMSEs),
