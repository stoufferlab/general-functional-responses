...
delcutoff <- unlist(lapply(ffr.fits, 
                               function(x){0.01*mean(x$study.info$data.Nconsumed)}))
delcutoff2 <- unlist(lapply(ffr.fits, 
                                function(x){0.01*mean(x$study.info$data.Nconsumed.mean)}))
delcutoff[which(is.na(delcutoff))] <- delcutoff2[which(!is.na(delcutoff2))]
