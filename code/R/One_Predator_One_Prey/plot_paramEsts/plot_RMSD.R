...
delRMSDcutoff <- unlist(lapply(ffr.fits, 
                               function(x){0.01*mean(x$study.info$data.Nconsumed)}))
delRMSDcutoff2 <- unlist(lapply(ffr.fits, 
...
# Rank order
#~~~~~~~~~~~
pdf('../../../../results/R/OnePredOnePrey_figs/OnePredOnePrey_RMSD_ranks.pdf',height=6,width=2.25)
  par(mar=c(3,7,2.5,0.5), mgp=c(1.5,0.2,0), tcl=-0.1, las=1, cex=0.7, yaxs='i')
    plot(1:nrow(rnkRMSDs), 1:nrow(rnkRMSDs),
         type='n', yaxt='n',
         xlim=c(1,ncol(rnkRMSDs)),
    ...
    box(lwd=1)
  par(xpd=TRUE)
    legend(-4,nrow(rnkRMSDs)+6,legend=colnames(rnkRMSDs),
