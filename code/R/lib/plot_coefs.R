...
plot.coefs <- function(
  ...
  if(is.null(ffr.fits) & is.null(ffr.profiled.fits)){
    stop('Must provide either "ffr.fits" or "ffr.profiled.fits"')
  }
  
  if(plot.SEs){
    if(is.null(ffr.profiled.fits)){
      stop('Profiles ("ffr.profiled.fits") must be provided to plot them')
    }else{
      ffr.fits <- ffr.profiled.fits
    }
  }
